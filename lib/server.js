	...
	for( var property in objYaml ){
		...
		if( objYaml.hasOwnProperty( property ) ){
			...
			if ( typeof objYaml[property] == "object" ){
				...
				var key = ( ( stack + '.' + property ).substring( 1 ) ).replace( /\./g , '\\.');
				googMap.set( key,
					{
						value: objYaml[property],
						re: new RegExp(  '\\[\\{\\(' + key + '\\)\\}\\]' , 'g' )
					} );
			}
		}
...
function getAppServer( ){
	...
	for( var i = 0; i < valReArray.length; i++ ){
		...
		tpl1.replace( valReArray[ i ].re, valReArray[ i ].value + '' );
		tpl2.replace( valReArray[ i ].re, valReArray[ i ].value + '' );
	}
...
function getTstServer( ){
	...
	for( var i = 0; i < valReArray.length; i++ ){
		...
		tpl.replace( valReArray[ i ].re, valReArray[ i ].value + '' );
	}
...
function startWebServer( ){
	...
	regExpsMap = getRegExpsMapForReplace( cfg, '', regExpsMap );
