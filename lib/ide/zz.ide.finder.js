...
DepsFileFinder.prototype.prepopulate_ = function( ) {
    ...
    for( var name in goog.dependencies_.nameToPath ){
        ...
        var filePath = path.resolve( this.projectDir_, goog.getPathFromDeps_( name ) );
        ...
        if( goog.string.startsWith( name, 'goog.' ) ){
            ...
            filePath = 'node_modules/imazzine-developer-kit/node_modules/google-closure-library/closure/goog/'
                + filePath;
        }

        this.files_[ name ] = filePath;

        if( name === 'goog.ds.XmlHttpDataSource' )
