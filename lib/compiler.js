    ...
    for( var key in json.dependencies ){
        ...
        if( !isSupportedNodePackage( key ) && ( !modulesMap.hasOwnProperty( key ) || modulesMap[key] !== 0 ) ){
            ...
            if( ft.isFileExist( nextModule + d + 'package.json' ) ) {
                ...
                isLibDir = ft.isDirExist( mod_root + d +
                    'node_modules' + d +
                    key + d +
                    'lib' );
                isSourceDir = isLibDir && ft.isDirExist( mod_root + d +
                    'node_modules' + d +
                    key + d +
                    'lib' + d +
                    'sources' );
                ...
                if( seeUp ){
                    ...
                    while( seeUp ){
                        ...
                        if( ft.isFileExist( rootUp + d + key + d + 'package.json' ) ){
                            ...
                            isLibDir = ft.isDirExist( rootUp + d +
                                key + d +
                                'lib' );
                            isSourceDir = isLibDir && ft.isDirExist( rootUp + d +
                                key + d +
                                'lib' + d +
                                'sources' );
...
function _compileJs( namespace, outPath ){
    ...
    try{
        ...
        if( ft.CONST.IS_WINDOWS_OS ){
            ...
            file = ft.openFile( outPath + d + namespace +'.js.map' );
            file = file.replace( new RegExp( ft.getRootPath( )
                .replace( /\\\\/g, '/' )
                .replace( /\\/g, '/' )
                , 'g' ), '' );
            ft.saveFile( outPath + d + namespace +'.js.map', file );
        }
