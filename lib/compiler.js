...
function getDepsRoot( ){
	...
	var res = '';
	var deps = ft.openYaml( ft.getRootPath() + d + 'dependencies.yaml' );
	for( var dep in deps ){

		var cfg = ft.openYaml( ft.getRootPath( ) + deps[ dep ] + d + 'config.yaml' );
		res = res + '--root="';
		res = res + ft.getRootPath( ) + deps[ dep ] + d + cfg.PATH.LIB + d + cfg.PATH.SOURCES;
		res = res + '" '
	}
	return res;
}
 ...
 * @returns {string}
 */
function getDepsWithPrefixRoot( ){

	var d = ft.CONST.PATH_DELIMITER;
	var res = '';
	var deps = ft.openYaml( ft.getRootPath() + ft.CONST.PATH_DELIMITER + 'dependencies.yaml' );
	for( var dep in deps ){
		...
		res = res +
			...
			ft.getRootPath( ) + deps[ dep ] + d + cfg.PATH.LIB + d + cfg.PATH.SOURCES +
			' ' +
			deps[ dep ] + '/' + cfg.PATH.LIB + '/' + cfg.PATH.SOURCES + '/" ';
	}
	return res;
}
...
function calculateDependencies( ){
	...
	var cmd =
		...
		// Dependency modules
			getDepsWithPrefixRoot( 'root_with_prefix' ) +
