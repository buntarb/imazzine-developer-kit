    ...
    while( queue[ level ] && queue[ level ].length ){
        ...
        for( i = 0; i < queue[ level ].length; i++ ){
            ...
            if( queue[ level ][ i ] && queue[ level ][ i ].children ) {
                ...
                if( queue[ level ][ i ][ 'properties' ] ){

                    for( j = 0; j < queue[ level ][ i ][ 'properties' ].length; j++ ){

                        if( queue[ level ][ i ][ 'properties' ][ j ][ 'type' ] ){

                            queue[ level ][ i ][ 'properties' ][ j ][ 'types' ] =

                                queue[ level ][ i ][ 'properties' ][ j ][ 'type' ][ 'names' ];

                            delete queue[ level ][ i ][ 'properties' ][ j ][ 'type' ];
                        }
                    }
                }
