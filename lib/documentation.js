...
function addMissingNodes( doc ){
    ...
    goog.array.forEach( doc, function( itemI, i){

        map[ itemI[ 'longname' ] ] = i;
        console.log( itemI[ 'longname' ], i );
        itemI[ 'children' ] = [ ];

        if( itemI[ 'memberof' ] ){

            var parentExist = false;
            goog.array.forEach( doc, function( itemJ ){

                if( !parentExist && itemI[ 'memberof' ] === itemJ[ 'longname' ] ){
            ...
            if( !parentExist ){
                ...
                var memberofValue = goog.array.slice(
                    ...
                    itemI[ 'memberof' ],
                    0,
                    itemI[ 'memberof' ].lastIndexOf( "." ) )
                    .toString( ).replace( /,/g, '');

                doc.push( {

                    name: ~itemI[ 'memberof' ].lastIndexOf( "." ) ?

                        nameValue :
                        itemI[ 'memberof' ],

                    longname: itemI[ 'memberof' ],
                    kind: "constant",
                    memberof: ~itemI[ 'memberof' ].lastIndexOf( "." ) ?
        ...
        for( i = 0; i < queue[ level ].length; i++ ){
            ...
            if( queue[ level ][ i ] && queue[ level ][ i ].children ) {
                ...
                if( queue[ level ][ i ][ 'meta' ] ){
                    ...
                    if( queue[ level ][ i ][ 'scope' ] === 'static' ){

                        queue[ level ][ i ][ 'enums' ] = queue[ level ][ i ][ 'meta' ][ 'code' ][ 'value' ];
                    }
                    if( queue[ level ][ i ][ 'scope' ] === 'instance' ){

                        if( queue[ level ][ i ][ 'params' ]){

                            queue[ level ][ i ][ 'kind' ] = 'function';

                        }else{

                            queue[ level ][ i ][ 'kind' ] = 'member';
                        }
                    }
    ...
    goog.array.forEach( queue, function( levelData ){
        ...
        goog.array.forEach( levelData, function( data ){
            ...
            if( data.path ){
                ...
                data.gitPath =  getPackageJson( data.path ).repository.url;
            }
        } );
    } );
    return queue;
